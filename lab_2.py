...
def find_dispersion(lst_y, average_y):
    dispersion_ydef generate_matrix():
    from random import randrange
    matrix_with_y = [[randrange(y_min, y_max) for y in range(m)] for x in range(3)]  # Генеруємо матрицю
    return list(matrix_with_y)
...
print("-"*40)
x1_min = int(input("Введіть x1 min: "))
x1_max = int(input("Введіть x1 max: "))
x2_min = int(input("Введіть x2 min: "))
x2_max = int(input("Введіть x2 max: "))
...
print(" ȳ1 = {:.3f}".format(average[0]))
print(" ȳ2 = {:.3f}".format(average[1]))
print(" ȳ3 = {:.3f}".format(average[2]))
print("-"*40)
print("\tДисперсії по рядках:")
print(" σ{y1} =" + " {:.3f}".format(dispersion[0]))
print(" σ{y2} =" + " {:.3f}".format(dispersion[1]))
print(" σ{y3} =" + " {:.3f}".format(dispersion[2]))
print("-"*40)
print("\t Ruv:")
print(" Ruv1 = {:.3f}".format(Ruv[0]))
print(" Ruv2 = {:.3f}".format(Ruv[1]))
print(" Ruv3 = {:.3f}".format(Ruv[2]))
print("-"*40)
...
print(" {:.3f} - {:.3f} + {:.3f} = {:.3f}".format(b0, b1, b2, b0 - b1 + b2))
print("Результат збігається  середніми значеннями ȳi.")
print("-"*40)
print("\tНатуралізоване рівняння регресії:\n ŷ = {:.3f} + {:.3f}*X1 + {:.3f}*X2".format(a0, a1, a2))
print("\tЗробимо перевірку по рядкам:\n {:.3f} + {:.3f}*{:.3f} + {:.3f}*{:.3f} = "
      "{:.3f}".format(a0, a1, x1_min, a2, x2_max, a0 + a1*x1_min + a2*x2_min))
print(" {:.3f} + {:.3f}*{:.3f} + {:.3f}*{:.3f} = {:.3f}".format(a0, a1, x1_max, a2, x2_min, a0 + a1*x1_max + a2*x2_min))
print(" {:.3f} + {:.3f}*{:.3f} + {:.3f}*{:.3f} = {:.3f}".format(a0, a1, x1_min, a2, x2_max, a0 + a1*x1_min + a2*x2_max))
print("Отже, коефіцієнти натуралізованого рівняння регресії вірні.")
print("-"*40)
1, dispersion_y2, dispersion_y3 = 0, 0, 0
    for k in range(m):
        dispersion_y1 += (lst_y[0][k] - average_y[0]) ** 2
        dispersion_y2 += (lst_y[1][k] - average_y[1]) ** 2
        dispersion_y3 += (lst_y[2][k] - average_y[2]) ** 2
